...
- Send heartbeat every 250 ms while the device is present, including idle. Confirm that local controls stay usable while idle, lock during motion/pause/calibration, local STOP still works once, and all remotely controlled relays deenergize after more than 10000 ms (10 s) without heartbeat. During remote motion, pause or calibration STATUS reports `HEARTBEAT_TIMEOUT` and `STOP_HEARTBEAT`; idle expiry after successful remote STOP keeps that successful result.
